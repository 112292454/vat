...
如果你是第一次接触 VAT，先不要从 Playlist、Watch、上传或 WebUI 开始。先按下面这条最短路径跑通 1 个本地视频：
...
# 生成新手可编辑的 starter config（不会直接复用作者机器路径/代理/凭据）
vat init

# 编辑 config/config.yaml，至少修改：
# 1) storage.work_dir / output_dir / database_path / models_dir
# 2) llm.api_key / llm.base_url（或设置 VAT_LLM_APIKEY）
# 3) 如不用下载/上传，可先保持 cookies 相关配置为空

# 用 1 个本地视频跑最小 happy-path
vat pipeline --url "/path/to/video.mp4" --title "demo"
```

说明：
- 推荐环境是 Linux + Python 3.10+ + CUDA GPU + 系统级 `ffmpeg`
- 如果只是第一次验证“能否跑通”，优先使用本地视频，避免把下载、Playlist、上传、WebUI 一起引入排障
- 第一次跑到 ASR 时，Whisper 模型会下载到 `storage.models_dir/asr.models_subdir`（默认 `./models/whisper`）；因此需要可用网络访问 HuggingFace，或提前把模型缓存放到该目录
- 更完整的配置说明见 [快速开始](#快速开始) 与 [CLI 使用](#cli-使用)
...
翻译引擎的核心实现参考自 [VideoCaptioner](https://github.com/WEIFENG2333/VideoCaptioner) 项目，包括：

- **反思翻译**：初译 → 反思 → 重译，显著提升翻译质量
...
- 更灵活的模型配置（支持不同阶段使用不同 LLM 服务商，当前生产配置：断句 gpt-4o-mini / 优化 kimi-k2.5 / 翻译 gemini-3-flash）
...
- LLM 视频信息自动翻译（标题、简介、标签），翻译后自动进行译名标准化（港台用语→大陆标准，如"マリオ"→"马里奥"而非"马力欧"）
- **直播视频处理**：三阶段策略（尝试 `live_from_start` 从头下载 → 等待直播结束 → VOD 下载），对上层流水线透明
- **Upcoming 视频**：支持提交尚未发布的视频任务，download 阶段自动轮询等待视频发布后再下载
...
- 自动获取封面、生成标签
- LLM 推荐 B 站分区
- 支持添加到合集（上传后自动添加到指定的 B站新版合集/SEASON），支持合集标题同步（用实际视频标题替换合集中的默认名称）
...
# 生成 starter config（需按你的机器环境继续编辑）
vat init
...
> `vat init` 生成的是**新手安全的 starter config**，不是作者当前机器的生产配置镜像。你仍然需要按自己的环境修改路径、LLM 配置，以及按需填写下载/上传 cookies。
...
Watch 模式是 VAT 的**全自动运行方式**：持续监控指定的 YouTube Playlist，发现新视频后自动完成「下载→识别→断句→优化→翻译→嵌入→上传→添加合集」全流程，无需人工干预。适用于需要时效性的场景（如 VTuber 直播结束后自动翻译上传）。

```bash
# 持续监控 Playlist（默认每 60 分钟检查一次）
vat watch -p PLAYLIST_ID

# 同时监控多个 Playlist，自定义间隔
...
**工作原理**：
- Watch 进程只负责**发现新视频并提交任务**，实际处理由 JobManager 子进程完成（复用 `vat process` 基础设施）
- 内置**跨进程资源锁**：多个 VAT 进程（watch 自动提交 + 用户手动执行）同时运行时，自动协调 YouTube 下载和 B站上传的速率，防止限流/风控
- 每轮检查自动过滤已处理、正在处理、不可用的视频，避免重复提交
- 失败的视频在后续轮次自动重试（同一视频最多重试 3 次）
- 支持通过 WebUI 的 Watch 管理页面启动/停止/查看状态

详细设计参见 [Watch Mode 文档](docs/WATCH_MODE_SPEC.md)。
...
<p align="center">
  <img src="docs/assets/webui_task_timer.png" alt="新建任务" width="90%">
</p>

### Watch 管理

独立的 Watch 管理页面，统一管理所有自动监控会话。可直接在 WebUI 中启动/停止 Watch 任务，查看每轮检查的发现视频数、提交任务数、错误信息等实时状态。Playlist 详情页也提供快捷的「开始监控」入口。

<p align="center">
  <img src="docs/assets/webui_watch.png" alt="Watch 管理" width="90%">
</p>

### 数据库浏览

WebUI 内置 SQLite 数据库只读浏览器（`/database`），可直接查看所有数据表的结构和内容，支持分页、搜索、排序、JSON 字段展开。用于开发调试和数据检查。

<p align="center">
  <img src="docs/assets/webui_database.png" alt="数据库浏览" width="90%">
</p>
...
│   ├── routes/           #   API 路由（视频/Playlist/任务/B站/提示词/Watch/数据库）
│   └── templates/        #   Jinja2 + TailwindCSS 页面模板
├── cli/                  # CLI 命令（click）
├── services/             # 业务逻辑层（Playlist 同步/Watch 服务/元信息管理）
├── subtitle_utils/       # 字幕工具（文本对齐、格式定义）
├── utils/                # 通用工具（GPU 管理、日志、缓存、输出验证、跨进程资源锁）
...
参数详解参见 [ASR 评估报告](docs/ASR_EVALUATION_REPORT.md)。

### GPU 分配

多 GPU 环境下的任务分配策略说明参见 [GPU 分配规范](docs/archive/gpu_allocation_spec.md)。
...
| 文档 | 内容 |
|------|------|
| [ASR 评估报告](docs/ASR_EVALUATION_REPORT.md) | Whisper 参数评估实验（350 个 VTuber 视频） |
| [翻译与 ASR 综合评测](docs/TRANSLATION_AND_ASR_EVALUATION.md) | 国产/海外 LLM 翻译对比、ASR 增强实验 |
| [Qwen3-Omni Captioner 评估](docs/qwen3_omni_caption.md) | 音频摘要模型的复现、扩展测试、结论与后续建议 |
| [提示词优化指南](docs/prompt_optimization_guide.md) | 翻译/优化提示词的编写方法 |
| [GPU 分配规范](docs/archive/gpu_allocation_spec.md) | 多 GPU 调度策略 |
...
| [多源下载设计](docs/archive/design_multi_source_download.md) | 本地文件/直链/YouTube 多源下载架构设计 |
| [已知问题](docs/known_issues.md) | 已知限制与 LLM 成本参考 |
| [Watch Mode 文档](docs/WATCH_MODE_SPEC.md) | Watch 模式设计与实现 |
| [ASR 增强实验](docs/ASR_ENHANCEMENT_EXPERIMENT.md) | 空 chunk 重试、分块参数调优等实验记录 |

---

## 相关工作

如果你对图结构与 Transformer 架构的结合感兴趣，欢迎查看我们的另一项研究工作：
...
探索如何将图结构数据有效转换为 Transformer 可处理的 token 序列。
