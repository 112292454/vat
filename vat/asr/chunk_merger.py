...
class ChunkMerger:
    ...
    def merge_chunks(
        ...
        """
        assert chunks and isinstance(chunks, list), "调用契约错误: chunks 不能为空且必须是列表"
        ...
        assert len(chunks) == len(chunk_offsets), \
            f"调用契约错误: chunks 数量 ({len(chunks)}) 与 chunk_offsets 数量 ({len(chunk_offsets)}) 不匹配"
        ...
        assert merged_segments, "逻辑错误: 合并后的 segments 为空"
        
        # 最终一致性检查：时间戳应该是递增的
        for i in range(1, len(merged_segments)):
            assert merged_segments[i].start_time >= merged_segments[i-1].start_time, \
                f"逻辑错误: 合并后时间戳非单调递增 (第 {i} 段 {merged_segments[i].start_time} < 前一段 {merged_segments[i-1].start_time})"
    ...
    def _merge_two_sequences(
        ...
        logger.info(
            ...
            f"初步切分点: left[:{left_cut}] + right[{right_mid}:]"
        )

        # 在选择切分点时就确保时间戳单调递增
        # 找到 left 和 right 的有效切分点组合
        final_left_cut = left_cut
        final_right_mid = right_mid
        
        if left_cut > 0 and right_mid < len(right):
            left_end_time = left[left_cut - 1].end_time
            right_start_time = right[right_mid].start_time
            
            if right_start_time < left_end_time:
                # 需要调整切分点以保证时间戳单调
                # 策略：在重叠区域内找到时间戳边界一致的切分点
                
                # 方案1: 向左移动 left_cut 直到 left[i].end_time <= right_start_time
                for i in range(left_cut - 1, -1, -1):
                    if left[i].end_time <= right_start_time:
                        final_left_cut = i + 1
                        break
                else:
                    final_left_cut = 0
                
                # 方案2: 向右移动 right_mid 直到 right[i].start_time >= left_end_time
                for i in range(right_mid, len(right)):
                    if right[i].start_time >= left_end_time:
                        alt_right_mid = i
                        break
                else:
                    alt_right_mid = len(right)
                
                # 选择丢弃更少片段的方案
                left_discard = left_cut - final_left_cut
                right_discard = alt_right_mid - right_mid
                
                if right_discard < left_discard:
                    # 使用方案2
                    final_left_cut = left_cut
                    final_right_mid = alt_right_mid
                # else: 使用方案1 (final_left_cut 已设置)
                
                logger.info(
                    f"调整切分点以保证时间戳单调: "
                    f"left[:{final_left_cut}] + right[{final_right_mid}:]"
                )
        
        return left[:final_left_cut] + right[final_right_mid:]
