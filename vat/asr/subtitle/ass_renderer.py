...
def render_ass_preview(
    ...
    try:
        # 自动换行处理
        processed_ass = auto_wrap_ass_file(temp_ass_path, fonts_dir=fonts_dir)

        # 确保背景图片存在
        bg_path_obj = Path(bg_image_path)
        if not bg_path_obj.exists():
            # 使用默认黑色背景
            default_bg = Path(resource_dir) / "assets" / "default_bg.png" if resource_dir else Path("/tmp/default_bg.png")
        ...
        # 生成预览图
        output_path = Path(cache_dir) / "ass_preview.png" if cache_dir else Path(tempfile.gettempdir()) / "ass_preview.png"
        ...
        # 添加内置字体目录支持
        fonts_dir_escaped = str(fonts_dir).replace("\\", "/").replace(":", r"\\:") if fonts_dir else ""
        ...
        cmd = [
            ...
            str(bg_path_obj),
            "-vf",
            f"ass={ass_file_escaped}:fontsdir={fonts_dir_escaped}" if fonts_dir else f"ass={ass_file_escaped}",
            "-frames:v",
            "1",
            str(output_path),
        ]
...
def render_ass_video(
    ...
    try:
        # 自动换行处理
        processed_subtitle = auto_wrap_ass_file(temp_ass_path, fonts_dir=fonts_dir)
        ...
        # 添加内置字体目录支持
        fonts_dir_escaped = Path(fonts_dir).as_posix().replace(":", r"\:") if fonts_dir else ""
