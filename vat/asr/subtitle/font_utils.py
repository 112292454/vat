...
@lru_cache(maxsize=1)
def get_builtin_fonts(fonts_dir: Optional[str] = None) -> tuple[Dict[str, str], ...]:
...
def get_font(size: int, font_name: str = "", fonts_dir: Optional[str] = None) -> FontType:
    """Get font object (built-in fonts first, then system fonts)"""
    if font_name:
        builtin_fonts = get_builtin_fonts(fonts_dir)
    ...
    for builtin in get_builtin_fonts(fonts_dir):
...
def get_ass_to_pil_ratio(font_name: str, fonts_dir: Optional[str] = None) -> float:
    """
