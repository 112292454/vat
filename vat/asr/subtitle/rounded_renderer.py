...
def render_subtitle_image(
    ...
    font = get_font(style.font_size, style.font_name, fonts_dir=fonts_dir)
...
def render_preview(
    ...
    subtitle_img = render_subtitle_image(
        primary_text, secondary_text, width, height, style, fonts_dir=fonts_dir
    )
...
def render_rounded_video(
    ...
    with tempfile.TemporaryDirectory(prefix="rounded_subtitle_") as temp_dir:
        ...
        for i, seg in enumerate(asr_data.segments):
            ...
            # 渲染字幕图片
            img = render_subtitle_image(primary, secondary, width, height, style, fonts_dir=fonts_dir)
