...
@cli.command('fix-thumbnails')
@click.option('--video-id', '-v', multiple=True, help='视频ID（可多次指定）')
@click.option('--all', 'fix_all', is_flag=True, help='扫描所有缺少封面的视频')
@click.option('--playlist', '-p', help='修复指定 Playlist 中缺少封面的视频')
@click.option('--workers', '-w', default=8, type=int, help='并发下载线程数（默认8）')
@click.option('--dry-run', is_flag=True, help='仅报告缺失情况，不下载')
@click.pass_context
def fix_thumbnails(ctx, video_id, fix_all, playlist, workers, dry_run):
    """补齐缺失的视频封面
    ...
    示例:
      vat fix-thumbnails --all              # 修复所有
      vat fix-thumbnails -v VIDEO_ID        # 修复指定视频
      vat fix-thumbnails -p PLAYLIST_ID     # 修复指定 Playlist
      vat fix-thumbnails --all --dry-run    # 仅报告
    """
    import json
    import re
    import requests
    ...
    base_dir = Path(config.storage.output_dir)
    ...
    proxy = config.downloader.proxy if hasattr(config.downloader, 'proxy') and config.downloader.proxy else None
    proxies = {"http": proxy, "https": proxy} if proxy else None

    # 收集目标视频
    if video_id:
        videos = [v for vid in video_id if (v := db.get_video(vid))]
    elif playlist:
        ps = PlaylistService(db)
        videos = ps.get_playlist_videos(playlist)
    elif fix_all:
        videos = db.list_videos()
    else:
        click.echo("请指定 --all、--video-id 或 --playlist")
        return

    # 筛选缺少本地封面的视频
    def has_local_cover(vid):
        out_dir = base_dir / vid
        if not out_dir.exists():
            return False
        return any((out_dir / f'{n}.{e}').exists() for n in COVER_NAMES for e in COVER_EXTS)

    missing = []
    for v in videos:
        if has_local_cover(v.id):
            continue
        meta = v.metadata or {}
    ...
    click.echo(f"扫描 {len(videos)} 个视频，缺少封面: {len(missing)} 个")

    if not missing:
        click.echo("所有视频都有本地封面 ✓")
        return
    ...
    if dry_run:
        for v in missing:
            ...
            click.echo(f"  缺失: {v.id}  URL={'有' if thumb else '无'}  title={v.title[:40] if v.title else '?'}")
        return

    def get_thumbnail_urls(v):
        ...
        yt_id = None
        if YT_ID_PATTERN.match(v.id):
            yt_id = v.id
        elif v.source_url:
            m = YT_URL_PATTERN.search(v.source_url)
            if m:
                yt_id = m.group(1)
    ...
    def download_one(v):
        out_dir = base_dir / v.id
        if not out_dir.exists():
            out_dir.mkdir(parents=True, exist_ok=True)

        candidate_urls = get_thumbnail_urls(v)
        ...
        for url in candidate_urls:
            try:
                ...
                (out_dir / f'thumbnail{ext}').write_bytes(resp.content)
                return v.id, 'ok', f'thumbnail{ext} ({len(resp.content)//1024}KB)'
            except Exception:
                continue

        return v.id, 'fail', 'all URLs failed'

    ok = fail = no_url = 0
    with ThreadPoolExecutor(max_workers=workers) as executor:
        futures = {executor.submit(download_one, v): v for v in missing}
        for i, future in enumerate(as_completed(futures), 1):
            ...
            if status == 'ok':
                ...
                no_url += 1
                click.echo(f"  跳过 {vid}: 无可用 URL")
            else:
                fail += 1
                click.echo(f"  失败 {vid}: {msg}")

            if i % 50 == 0 or i == len(missing):
                click.echo(f"  [{i}/{len(missing)}] 成功={ok} 失败={fail} 无URL={no_url}")
