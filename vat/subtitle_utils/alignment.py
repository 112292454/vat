...
class SubtitleAligner:
    ...
    def align_texts(self, source_text, target_text):
        ...
        """
        assert isinstance(source_text, list), "调用契约错误: source_text 必须是列表"
        assert isinstance(target_text, list), "调用契约错误: target_text 必须是列表"
        
        diff_iterator = difflib.ndiff(source_text, target_text)
        aligned_source, aligned_target = self._pair_lines(diff_iterator)
        
        assert len(aligned_source) == len(aligned_target), \
            f"逻辑错误: 对齐后列表长度不一致 ({len(aligned_source)} vs {len(aligned_target)})"
