...
def str_to_target_language(value: str) -> TargetLanguage:
    """
    将字符串转换为 TargetLanguage 枚举
    支持枚举值（中文名）、枚举名（英文大写）、语言代码（zh-cn, en, etc.）
    """
    ...
    # 2. 尝试作为枚举名（英文大写）
    try:
        return TargetLanguage[value.upper()]
    except KeyError:
        pass
    ...
    if value_lower in code_map:
        return code_map[value_lower]

    # 从现有的映射表中查找
    for lang_map in [GOOGLE_LANG_MAP, BING_LANG_MAP, DEEPL_LANG_MAP]:
        for lang_enum, code in lang_map.items():
            if code.lower() == value_lower:
                return lang_enum

    # 如果都失败了，抛出原始异常信息
    valid_values = [e.value for e in TargetLanguage]
    raise ValueError(f"'{value}' is not a valid TargetLanguage. Please use one of: {valid_values}")
